...
from models import User, DEVELOPER_USERNAME, DEVELOPER_PASSWORD_HASH
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(API_URL, json=payload, headers=headers, timeout=timeout) as response:
            if response.status != 200:
                ...
                raise APIRequestError(f"API Error: {response.status} - {error_text[:100]}")
            data = await response.json()
            return data["choices"][0]["message"]["content"].strip()

def init_admin_user(db, username, password_hash):
    if not User.query.filter_by(username=username).first():
        admin = User(username=username)
        admin.password_hash = password_hash
        admin.premium_until = datetime.utcnow() + timedelta(days=9999)
